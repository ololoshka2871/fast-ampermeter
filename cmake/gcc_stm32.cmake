...
SET(STM32_SUPPORTED_FAMILIES L0 F0 F1 L1 F2 F3 F4 L4 CACHE INTERNAL "stm32 supported families")
...
IF(NOT STM32_FAMILY)
    ...
    ELSE()
        STRING(REGEX REPLACE "^[sS][tT][mM]32(([fF][0-4])|([lL][0-14])|([tT])|([wW])).+$" "\\1" STM32_FAMILY ${STM32_CHIP})
...
SET(CMAKE_READELF ${TOOLCHAIN_BIN_DIR}/${TARGET_TRIPLET}-readelf${TOOL_EXECUTABLE_SUFFIX} CACHE INTERNAL "readelf tool")
...
if (BUILD_USE_LTO)
    set(CFLAGS_COMMON ${CFLAGS_COMMON} -flto)
    message(STATUS "Using LTO (-flto)")
endif(BUILD_USE_LTO)

# no lto for asm targets!
SET(CMAKE_C_FLAGS_DEBUG "-Os -g ${CFLAGS_COMMON}" CACHE INTERNAL "c compiler flags debug")
SET(CMAKE_CXX_FLAGS_DEBUG "-Os -g ${CFLAGS_COMMON}" CACHE INTERNAL "cxx compiler flags debug")
SET(CMAKE_ASM_FLAGS_DEBUG "-g ${CFLAGS_COMMON}" CACHE INTERNAL "asm compiler flags debug")
SET(CMAKE_EXE_LINKER_FLAGS_DEBUG ${CFLAGS_COMMON} CACHE INTERNAL "linker flags debug")

SET(CMAKE_C_FLAGS_RELEASE "-Os -g -DNDEBUG ${CFLAGS_COMMON}" CACHE INTERNAL "c compiler flags release")
SET(CMAKE_CXX_FLAGS_RELEASE "-Os -g -DNDEBUG ${CFLAGS_COMMON}" CACHE INTERNAL "cxx compiler flags release")
SET(CMAKE_ASM_FLAGS_RELEASE "-g -DNDEBUG ${CFLAGS_COMMON}" CACHE INTERNAL "asm compiler flags release")
SET(CMAKE_EXE_LINKER_FLAGS_RELEASE ${CFLAGS_COMMON} CACHE INTERNAL "linker flags release")
...
FUNCTION(STM32_GENERATE_MAP_FILE TARGET)
    IF(EXECUTABLE_OUTPUT_PATH)
      SET(FILENAME "${EXECUTABLE_OUTPUT_PATH}/${TARGET}")
    ELSE()
      SET(FILENAME "${TARGET}")
    ENDIF()
    MESSAGE(STATUS "Adjusting linker flags to enable map file generation")
    SET(TARGET_LD_FLAGS "${TARGET_LD_FLAGS} -Wl,-Map,${FILENAME}.map  -Wl,--cref")
...
FUNCTION(STM32_SET_TARGET_PROPERTIES TARGET)
    ...
    IF((NOT STM32_FLASH_SIZE) OR (NOT STM32_RAM_SIZE))
        STM32_GET_CHIP_PARAMETERS(${STM32_CHIP} STM32_FLASH_SIZE STM32_RAM_SIZE CC_RAM_SIZE)
        IF((NOT STM32_FLASH_SIZE) OR (NOT STM32_RAM_SIZE))
