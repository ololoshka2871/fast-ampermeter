...
template <typename Ret, typename... Args> struct result_of<Ret(Args...)> {
  typedef Ret type;
};
} // namespace impl
...
template <typename R> struct ResultOkType {
  typedef typename std::decay<R>::type type;
};

template <typename T, typename E> struct ResultOkType<Result<T, E>> {
  typedef T type;
};

template <typename R> struct ResultErrType { typedef R type; };

template <typename T, typename E> struct ResultErrType<Result<T, E>> {
...
template <typename Ret, typename Arg> struct Map<Ret(Arg)> {
...
template <typename Arg> struct Map<void(Arg)> {
...
template <typename Ret> struct Map<Ret(void)> {
...
template <typename U, typename E, typename Arg> struct Map<Result<U, E>(Arg)> {
...
template <typename U, typename E> struct Map<Result<U, E>(void)> {
...
template <typename Ret, typename Arg> struct Then<Ret(Arg)> {
...
template <typename Ret> struct Then<Ret(void)> {
  static_assert(std::is_same<Ret, void>::value,
                "then() should not return anything, use map() instead");
...
template <typename T, typename F, typename Arg> struct Else<Result<T, F>(Arg)> {

  template <typename E, typename Func>
  static Result<T, F> orElse(const Result<T, E> &result, Func func) {
    static_assert(std::is_same<E, Arg>::value ||
...
template <typename T, typename F> struct Else<Result<T, F>(void)> {
...
template <typename Ret, typename Arg> struct Wise<Ret(Arg)> {
...
template <
    typename T, typename E, typename Func,
    typename Ret = Result<typename details::ResultOkType<
...
template <typename T, typename E, typename Func,
          typename Ret =
              Result<T, typename details::ResultErrType<
                            typename details::result_of<Func>::type>::type>>
...
template <typename T, typename E> struct Storage {
  ...
  template <typename U> void rawConstruct(U &&val) {
...
template <typename T, typename E> struct Constructor {
...
template <typename E> struct Constructor<void, E> {
  ...
  static void copy(const Storage<void, E> &src, Storage<void, E> &dst,
                   err_tag) {
    dst.rawConstruct(src.template get<E>());
  }
};
...
namespace concept {
  ...
  struct EqualityComparable<
      T, typename std::enable_if<
             true, typename details::void_t<decltype(
...
template <typename T, typename E> struct Result {
  ...
  Result(Result &&other) {
    if (other.isOk()) {
      details::Constructor<T, E>::move(std::move(other.storage_), storage_,
                                       details::ok_tag());
      ok_ = true;
    } else {
      details::Constructor<T, E>::move(std::move(other.storage_), storage_,
  ...
  Result(const Result &other) {
    if (other.isOk()) {
      details::Constructor<T, E>::copy(other.storage_, storage_,
                                       details::ok_tag());
      ok_ = true;
    } else {
      details::Constructor<T, E>::copy(other.storage_, storage_,
  ...
  template <typename Func> Result<T, E> then(Func func) const {
    return details::then(*this, func);
  }

  template <typename Func> Result<T, E> otherwise(Func func) const {
